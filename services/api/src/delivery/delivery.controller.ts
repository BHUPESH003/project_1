...
export class DeliveryController {
  ...
  @ApiOperation({
    ...
    description:
      'Standalone endpoint for pickup/drop location bookings. Returns all available delivery provider options with pricing, estimated delivery time, and ratings. No order required.',
  })
  @ApiResponse({
    status: 200,
  ...
  @ApiResponse({
    status: 400,
    description: 'Invalid request parameters or location coordinates',
  })
  @ApiResponse({
    status: 503,
    description: 'No delivery partners available for this location',
  })
  ...
  @ApiOperation({
    ...
    description:
      'Assigns a delivery partner to an order. Called automatically when order reaches READY_FOR_PICKUP state.',
  })
  ...
  @ApiOperation({
    ...
    description:
      'Receives delivery status updates from delivery provider. Idempotent - duplicate webhooks are safely ignored. Order state transitions via Order State Machine.',
  })
  @ApiHeader({
